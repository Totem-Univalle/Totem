...
function AdminRegistrationForm() {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://localhost:7264/api/Totems', formData)
      .then(response => {
  ...
  return (
   <>
    ...
    <form onSubmit={handleSubmit}>
      <div>
        <div>
          <label className="text-gray-700 dark:text-black-200" htmlFor="firstName">
            Nombre
          </label>
          <input
        ...
        <input type="number" value={NumeroPlantilla} onChange={(e) => setNumeroPlantilla(e.target.value)} />
      </div>
      <div>
        <label>Usuario id:</label>
        <input type="number" value={Usuario} onChange={(e) => setUsuario(e.target.value)} />
      </div>
